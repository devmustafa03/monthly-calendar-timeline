...
const CalendarHeader: React.FC<CalendarProps> = ({onAddResource}) => {
  ...
  return (
    <header className="flex justify-between items-center p-4 bg-gray-100">
      <h1 className="text-2xl font-bold">{state.currentDate.format('MMMM YYYY')}</h1>
      ...
      <button onClick={() => onAddResource()} className="px-4 py-2 bg-green-500 text-white rounded">
        ...
        {/* Today */}
        <button onClick={() => dispatch({ type: 'SET_CURRENT_DATE', payload: dayjs() })} className="px-4 py-2 text-cyan-600 font-bold rounded">
          Today
        </button>
        {/* < Next > */}
